#!/usr/bin/env python3
"""
citation_audit.py - 学术论文引用一致性审计工具 (Version 1.0)
...
def extract_citations_from_text(text, page_num=0):
    ...
    _EN_PREFIX_RE = re.compile(r'^(?:e\.g\.|i\.e\.|see|cf\.)\s*,?\s*', re.I)
    for m in re.finditer(r'[（\(]([^（）\(\)]+)[）\)]', text):
        inner = m.group(1).strip()
        if ';' not in inner and '；' not in inner:
            continue
        for part in re.split(r'[;；]', inner):
            part = _EN_PREFIX_RE.sub('', part).strip()
            if not part:
                continue
            # 优先匹配 "Author and Author Year"
            m2 = re.match(
                r"([A-Z][a-zA-Z'\-]+)\s+(?:and|&)\s+([A-Z][a-zA-Z'\-]+)\s*(\d{4})\s*$", part
            )
            if m2:
                citations.append({
            ...
            m2 = re.match(
                r"([A-Z][a-zA-Z'\-]+(?:\s+[A-Z][a-zA-Z'\-]+)?(?:\s+et\s+al\.?)?)\s*(\d{4})\s*$", part
            )
            if m2:
                author = clean_author(m2.group(1))
                # 过滤以 "The" 开头的非作者名
                ...
                citations.append({
                    ...
                    'author_count': 'multi' if _has_etal(part) else 'single',
                    'author2': None
                })
    ...
    for m in re.finditer(
        r"[（\(]\s*([A-Z][a-zA-Z'\-]+(?:\s+[A-Z][a-zA-Z'\-]+)?(?:\s+et\s+al\.?)?)\s*[，,]?\s*(\d{4})\s*[）\)]",
        text
    ):
        raw = m.group()
    ...
    for m in re.finditer(
        r"[（\(]\s*([a-z][a-zA-Z'\-]+\s+[A-Z][a-zA-Z'\-]+(?:\s+[A-Z][a-zA-Z'\-]+)?(?:\s+et\s+al\.?)?)\s*[，,]?\s*(\d{4})\s*[）\)]",
        text
    ):
        raw = m.group()
        author = clean_author(m.group(1))
    ...
    for m in re.finditer(
        r"[（\(]\s*([A-Z][a-zA-Z'\-]+(?:\s+[A-Z][a-zA-Z'\-]+)?(?:\s*[,&]\s*[A-Z][a-zA-Z'\-]+(?:\s+[A-Z][a-zA-Z'\-]+)?)*)\s*[，,]?\s*(\d{4})\s*[）\)]",
        text
    ):
        raw = m.group()
    ...
    for m in re.finditer(
        r"(?<![A-Za-z])([A-Z][a-zA-Z'\-]+)\s+(?:and|&)\s+([A-Z][a-zA-Z'\-]+)\s*[（\(]\s*(\d{4})\s*[）\)]",
        text
    ):
        citations.append({
    ...
    # 注意: 叙事模式没有括号锚定，多词姓会误捕获机构名，仅支持单词姓
    for m in re.finditer(
        r"(?<![A-Za-z])([A-Z][a-zA-Z'\-]+(?:\s+et\s+al\.?)?)\s*[（\(]\s*(\d{4})\s*[）\)]",
        text
    ):
        raw = m.group()
        author = clean_author(m.group(1))
...
def match_via_canonical(citations, ref_entries, fuzzy=False):
    ...
    for c in citations:
        ...
        # "eyal and liberman 2012" → author_part="eyal and liberman"
        #   只匹配 ref 的前两位也是 "eyal and liberman" 但年份不同的条目
        # "yang 2018" → author_part="yang"
        #   只匹配 first_author="yang" 但年份不同的条目（不匹配合作者 yang）
        ...
        if author_part in ym_index:
            for ri, ref_year in ym_index[author_part]:
                if ref_year != cy:
                    year_mismatches.append({
                        'citation': c, 'reference': ref_entries[ri],
        ...
        # 作者不匹配检测：同一年份，作者名近似但不一致
        for ri, ref in enumerate(ref_entries):
            if ref['year'] != cy:
                continue
            ref_a1 = author_normalize(ref['first_author'])
            ratio = _levenshtein_ratio(a1, ref_a1)
            if 0.3 < ratio < 0.7:
                author_mismatches.append({
                    'citation': c, 'reference': ref,
                    'cite_author': c['author'], 'ref_author': ref['first_author']
                })
                break
...
def find_cross_year_warnings(missing, ref_entries):
    ...
    for c in missing:
        ...
        best_ratio = 0
        for r in ref_entries:
            for au in r.get('all_authors', []):
                ratio = _levenshtein_ratio(a1, author_normalize(au))
                if ratio > best_ratio and r['year'] != cy:
                    best_ratio = ratio
...
def generate_report(pdf_path, citations, ref_entries, missing, uncited,
    ...
    lines.append('  3. 仅匹配(第一作者, 年)，同名同年的多篇文献无法区分')
...
def generate_html_report(pdf_path, citations, ref_entries, missing, uncited,
                         year_mismatches, author_mismatches, duplicates, elapsed,
    ...
    def _issue_rows(items, label, severity, type_key):
        ...
        for i, item in enumerate(items, 1):
            ...
            if type_key == 'missing':
                ...
                rows += f'<tr style="background:{color}10"><td>{i}</td><td>{auth}</td><td>{item["year"]}</td><td>R{item["id"]}</td><td><span class="badge" style="background:{color}">{severity}</span></td></tr>\n'
            ...
            elif type_key == 'author_mismatch':
                rows += f'<tr style="background:{_sev_color("HIGH")}10"><td>{i}</td><td>{item["cite_author"]}</td><td>{item["citation"]["year"]}</td><td>正文: {item["cite_author"]}<br>文献: {item["ref_author"]}</td><td><span class="badge" style="background:{_sev_color("HIGH")}">HIGH</span></td></tr>\n'
...
<div class="summary-card"><div class="num" style="color:#dc3545">{len(author_mismatches)}</div><div class="label">作者不匹配</div></div>
...
<p class="text-muted">正文引用年份与参考文献年份不一致</p>
...
{_issue_rows(year_mismatches,'year','HIGH','year_mismatch')}
    ...
    if author_mismatches:
        html += f'''<div class="card">
<h2 style="color:#dc3545">🔴 作者不匹配 ({len(author_mismatches)})</h2>
<p class="text-muted">正文引用作者与参考文献作者不一致（但年份相同）</p>
<table><thead><tr><th>#</th><th>正文作者</th><th>年份</th><th>差异详情</th><th>严重度</th></tr></thead><tbody>
{_issue_rows(author_mismatches,'author','HIGH','author_mismatch')}
</tbody></table></div>'''
...
<li>仅匹配(第一作者, 年份)，同名同年的多篇文献无法区分</li>
...
def compute_quality_stats(citations, ref_entries, missing, uncited,
                           year_mismatches, author_mismatches, duplicates,
                           style=""):
    """
    计算引用质量量化指标。
    ...
    return {
        ...
        'author_mismatch_count': len(author_mismatches),
...
def main():
    ...
    # 5 — 匹配
    print('  [5/5] 执行双向匹配...')
    missing, uncited, year_mismatches, author_mismatches = match_via_canonical(
    ...
    stats = compute_quality_stats(
        ...
        year_mismatches, author_mismatches,
        duplicates, style
    )
    ...
    if args.ai:
        ...
        print('  [AI] 加载 AI 分析引擎...')
        try:
            from citation_ai_v1_0 import LMStudioClient, ai_do_full_review, ai_generate_review_summary
            ...
            if client.is_available():
                ...
                if args.ai_full:
                    ...
                    from citation_ai_v1_0 import ai_recover_references_batch
